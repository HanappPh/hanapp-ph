...
export function ProviderCategories({
  ...
  const handleCategoryClick = (categoryId: string) => {
    ...
    // Redirect to categories page with the selected category (same as client side)
  ...
  const handleListServiceClick = () => {
    ...
    router.push('/provider/jobs/create');
  };

  return (
    <section className="container mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-[#102E50]">Categories</h2>
        <Button
          variant="outline"
          size="sm"
          className="bg-[#F5C45E] hover:bg-[#FFDD8E] border-none text-[#102E50] font-semibold"
      ...
      <div className="flex items-center gap-4">
        ...
        >
          <ChevronLeft className="w-5 h-5 text-[#102E50]" />
        </button>
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center p-4 rounded-2xl transition-all ${
                          isSelected
                            ? 'bg-white border-2 border-[#102E50] shadow-md'
                            : 'bg-[#FFF5D6] border-2 border-transparent'
                        }`}
                      >
                        <div className="flex items-center justify-center w-14 h-14 bg-white rounded-full mb-2">
                          <Icon className="w-7 h-7 text-[#102E50]" />
                        </div>
                        <span className="text-xs font-medium text-[#102E50] text-center leading-tight">
                          {category.name}
                        </span>
                      </button>
                    );
        ...
        >
          <ChevronRight className="w-5 h-5 text-[#102E50]" />
        </button>
      </div>
        ...
        {Array.from({ length: totalPages }).map((_, index) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-colors ${
              currentPage === index ? 'bg-[#102E50]' : 'bg-gray-400'
            }`}
